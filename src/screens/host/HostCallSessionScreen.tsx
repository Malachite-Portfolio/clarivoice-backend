...
type Props = NativeStackScreenProps<RootStackParamList, "HostCallSession">;

export function HostCallSessionScreen({ route, navigation }: Props) {
  const { userAvatarUrl, userId, userName, callId } = route.params;
  ...
  const activeDuration = useMemo(() => callDuration(call?.durationSec ?? 0), [call?.durationSec]);
  const earnedCoins = call?.chargedCoins ?? (call?.billedMinutes ?? 0) * 50;
  ...
  const ensureMicPermission = useCallback(async () => {
    ...
    Alert.alert(
      ...
      nextStatus === "blocked"
        ? "Microphone access is blocked. Open app settings to accept voice calls."
        : "Microphone access is needed before joining this call.",
      [
  ...
  const initCall = useCallback(async () => {
    ...
    try {
      ...
      if (!hasMicPermission) {
        setError("Microphone access is required before joining a call.");
        return;
      }

      await activateVoiceCallAudio(false);
      if (callId) {
        const calls = await fetchCalls(hostId, apiBaseUrl, undefined, 40, "host");
        const found = calls.items.find((item) => item.id === callId) ?? null;
        setCall(found);
      } else {
        const started = await startCall(hostId, userId, apiBaseUrl, "host");
        setCall(started);
      }
    } catch (initError) {
      setError(initError instanceof Error ? initError.message : "Could not start host call.");
    } finally {
      setLoading(false);
    }
  ...
  async function handleEndCall() {
    ...
    try {
      const ended = await updateCallState(call.id, hostId, "ended", apiBaseUrl, "host");
      setCall(ended);
  ...
  async function handleCallAgain() {
    ...
    try {
      ...
      setError(startError instanceof Error ? startError.message : "Could not call user again.");
    }
  }
  ...
  return (
    <SafeAreaView style={styles.root} edges={["top"]}>
      <View style={styles.topBar}>
        ...
        <Text style={styles.topTitle}>Host Audio Call</Text>
      ...
      <View style={styles.center}>
        ...
        <Text style={styles.earnings}>Call earnings: {earnedCoins} coins</Text>
